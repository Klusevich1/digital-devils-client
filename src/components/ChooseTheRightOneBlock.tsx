...
export const ChooseTheRightOneBlock: FC<{ title: string; text1: ReactNode; text2: ReactNode; slash: string[], card: string, data: {title: string, text: string}[] }> = ({ title, text1, text2, slash, card, data }) => {
    return (
                    ...
                    <div  className="max-w-[640px] text-[18px]  md:w-1/2">
                        {slash && slash.length ===  0 ? <></> :
                            <ul className="my-[20px]">
                                {slash.map((item, idx) => (
                                    <li key={idx}><span className="text-custom_yellow text-[18px] font-medium">/</span> {item}</li>
                                ))}
                            </ul>
                        }
